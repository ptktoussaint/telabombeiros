  ...
  if (live) {
    for (const viewer of live.viewers.values()) {
      socket.emit('viewer:joined', {
        ...
        label: viewer.label,
        isAdminMonitor: viewer.isAdminMonitor,
      });
    }
  }
...
async function setupViewer(io, socket, session) {
  ...
  notifyTransmitter(io, roomId, 'viewer:joined', { roomId, viewerId, label, isAdminMonitor: false });
...
module.exports = { attachSignaling, ADMIN_ROOM, ADMIN_MONITOR_LABEL };
