...
class LiveState extends EventEmitter {
  ...
  // Quem o dono da sala desconectou fica barrado ate a sala acabar. E so em memoria
  // de proposito: nao e punicao registrada, e uma remocao daquela transmissao.
  blockSession(roomId, sessionId) {
    if (!sessionId) return false;
    const room = this.rooms.get(String(roomId));
    if (!room) return false;
    room.blockedSessions.add(String(sessionId));
    return true;
  }

  isSessionBlocked(roomId, sessionId) {
    if (!sessionId) return false;
    const room = this.rooms.get(String(roomId));
    if (!room) return false;
    return room.blockedSessions.has(String(sessionId));
  }

  unblockSession(roomId, sessionId) {
    const room = this.rooms.get(String(roomId));
    if (!room) return false;
    return room.blockedSessions.delete(String(sessionId));
  }
